...
begin
  ...
  case PTypeInfo(TypeInfo(T))^.Kind of
    tkInteger:
      Result := T(JsonData.AsInteger);
    tkInt64:  // Adicionado suporte para Int64
      Result := T(JsonData.AsInt64);
    tkFloat:
      Result := T(JsonData.AsFloat);
    tkUString, tkString:
      Result := T(JsonData.AsString);
    tkBool:
      Result := T(JsonData.AsBoolean);
    else
      raise Exception.Create('Tipo genérico não suportado');
  end;
end;
